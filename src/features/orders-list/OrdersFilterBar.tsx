...
export default function OrdersFilterBar({ value, onChange }: Props) {
  ...
  const assigneeOptions = useMemo(
    () => [{ id: "", full_name: "Any assignee" }, ...users],
    [users]
  );

  const clientOptions = useMemo(
    () => [{ id: "", display_name: "Any client" }, ...clients],
    [clients]
  );

  return (
    <div className="flex flex-col gap-3 mb-4">
      {/* Row 1: status chips + text search */}
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          {assigneeOptions.map(u => (
            <option key={u.id || "any"} value={u.id}>{u.full_name ?? "Any assignee"}</option>
          ))}
        </select>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {clientOptions.map(c => (
              <option key={c.id || "any"} value={c.id}>{c.display_name}</option>
            ))}
          </select>
        </div>
        ...
        <input
          ...
          className="border rounded px-2 py-1"
          value={value.dueFrom ?? ""}
          onChange={(e) => onChange({ ...value, dueFrom: e.target.value || null, page: 1 })}
        />
        ...
        <input
          ...
          className="border rounded px-2 py-1"
          value={value.dueTo ?? ""}
          onChange={(e) => onChange({ ...value, dueTo: e.target.value || null, page: 1 })}
        />
      </div>
    </div>
  );
